...
for row in $(jq -r 'to_entries[] | @base64' subdomain.json); do
  ...
  DNS_RECORD_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$ZONE_ID/dns_records?name=$SUBDOMAIN.yourdomain.com&type=CNAME" \
    -H "Authorization: Bearer $API_TOKEN" \
    -H "Content-Type: application/json" | jq -r '.result[0].id')

  # DNS 레코드가 있으면 업데이트, 없으면 생성
  if [ "$DNS_RECORD_ID" != "null" ]; then
    echo "Updating existing CNAME record for $SUBDOMAIN"
  ...
  echo "CNAME for $SUBDOMAIN -> $TARGET has been applied."
done
